...
const DashboardPanel = ({ title, content }) => (
  <div className="bg-blue-900 bg-opacity-40 backdrop-blur-md p-8 rounded-2xl border border-blue-300 border-opacity-20 
                  transition-all duration-300 ease-in-out
                  hover:bg-opacity-60 hover:shadow-lg hover:shadow-blue-500/50
                  hover:-translate-y-1 hover:scale-105
                  transform perspective-1000">
    <h2 className="text-2xl font-semibold mb-4 text-blue-200">{title}</h2>
...
const AdminDashboard = () => {
  ...
  const notifications = [
    { title: "New Registration", content: "5 new delegates have registered." },
    { title: "Schedule Update", content: "The conference schedule has been updated." },
    { title: "Resource Request", content: "New resource request from Committee A." },
  ];
  ...
  const handleInfoClick = () => {
    alert('This is the QTMUN Admin Portal. Here you can manage announcements, delegate information, schedules, and resources.');
  };
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-900 via-blue-800 to-purple-900 p-4 animate-fadeIn">
      <div className="w-full max-w-6xl bg-blue-950 bg-opacity-30 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-blue-400 border-opacity-20">
        <header className="flex justify-between items-center mb-8">
          <h1 className="text-4xl font-bold text-white">QTMUN Admin Portal</h1>
          <div className="flex items-center space-x-4">
            ...
            <div className="relative dropdown-container">
              <button 
                ...
                className="w-12 h-12 bg-gray-300 rounded-full overflow-hidden focus:outline-none focus:ring-2 focus:ring-blue-300"
              >
                <img src="/api/placeholder/48/48" alt="Admin portrait" className="w-full h-full object-cover" />
              </button>
              <div className={`absolute right-0 mt-2 w-56 bg-white rounded-md shadow-lg py-1 z-10 transition-all duration-300 ease-in-out ${activeDropdown === 'userMenu' ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
                <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <DashboardPanel 
            ...
            content="Post and manage announcements for delegates and participants." 
          />
          <DashboardPanel 
            title="Delegate Information" 
            content="View and edit delegate details and registrations." 
          />
          <DashboardPanel 
            title="Event Schedule" 
            content="Update and manage the conference schedule." 
          />
          <DashboardPanel 
            title="Resource Management" 
            content="Manage and distribute resources for the event." 
          />
        </div>
